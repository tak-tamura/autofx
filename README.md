...
戦略シグナルは確定足で判定し、次の足の始値で約定します。ストップロスと利益確定は足の高値・安値で判定し、同一足で両方へ到達した場合は保守的にストップロスを優先します。価格が決済水準を越えて始まった場合は、その足の始値で決済します。データ末尾の未決済ポジションは最終足終値で強制決済されます。

チャートAPIのバックテストレスポンスには、シグナル時刻、決済理由、およびこれらの約定仮定が含まれます。現時点ではスプレッド、スリッページ、手数料は明示的にゼロです。
